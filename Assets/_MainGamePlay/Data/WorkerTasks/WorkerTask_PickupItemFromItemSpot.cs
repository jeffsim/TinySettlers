...
public class WorkerTask_PickupItemFromStorageSpot : WorkerTask
{
    ...
    public override string ToDebugString()
    {
        ...
        switch (substate)
        {
            case (int)WorkerTask_PickupItemFromStorageSpotSubstate.GotoItemSpotWithItem: str += "; dist: " + Vector2.Distance(Worker.WorldLoc, spotWithItemToPickup.WorldLoc).ToString("0.0"); break;
    ...
    public override void OnBuildingDestroyed(BuildingData building)
    {
        ...
        if (spotWithItemToPickup.Building.IsDestroyed && substate == 0)
            Abandon();
        else if (reservedSpotToStoreItemIn.Building.IsDestroyed && substate == 0)
            Abandon();
    }
